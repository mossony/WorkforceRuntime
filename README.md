...
Run a large-organization scale smoke test that initializes 3,000 agents,
enqueues one `worker_run` item for each worker, and only claims a bounded
...
dashboard behavior, worker launch settings, benchmark defaults, or LLM provider
...
`OPENROUTER_API_KEY` and `NVIDIA_API_KEY`; do not write API keys into the JSON
config.

Worker execution mode is also controlled from the same JSON file. The default
`execution.mode` is `full_access`, which preserves the current behavior. Setting
it to `sandbox` prepends the configured `execution.sandbox.command_prefix` to
...
The current organization tools are `report`, `assign`, `discuss`, `check_progress`, `hire`, and `update_system_prompt`. The worker support tools are `submit_artifact`, `update_status`, `request_budget`, `request_permission`, `get_task_context`, and `get_org_context`. Queue coordination tools are `enqueue_work`, `claim_work`, `complete_work`, `fail_work`, and `get_work_queue`; they coordinate `llm_request`, `tool_call`, and `worker_run` items under active-agent, model, tool, and kind limits. See `MCP_TOOLS.md`.
...
For stronger isolation, set `execution.mode` to `sandbox` in
`workforce_runtime_config.json`. The default template uses `srt --settings
examples/sandbox_runtime_settings.json` as the process wrapper and adds
Claude's bypass-permissions flag only inside that external sandbox. You can
replace the command prefix with Docker, Docker Sandboxes, devcontainers, or an
internal wrapper without changing worker code.
...
- `docs/STORAGE_BACKENDS.md`: runtime storage abstraction and future backend adapter notes.
