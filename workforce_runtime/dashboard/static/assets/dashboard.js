...
const dashboardShell = `<div id="app-shell">
  ...
  <aside id="sidebar">
    ...
    <div class="sb-settings-links">
      <button class="sb-settings-button" data-action="open-settings" data-settings-view="mcp">
        <span class="sb-settings-icon">M</span>
        <span>MCP</span>
      </button>
      <button class="sb-settings-button" data-action="open-settings" data-settings-view="skills">
        <span class="sb-settings-icon">S</span>
        <span>Skills</span>
      </button>
    </div>
  ...
  <main id="main">
    ...
    <div class="wr-scroll" id="main-scroll">
      <div id="main-content">
        ...
        <!-- TITLE -->
        <div class="home-main-block" style="padding:48px 0 22px 0;">
        ...
        <!-- INPUT CARD -->
        <div class="input-card idle home-main-block" id="input-card">
          <div class="input-top">
            <button class="input-attach" data-action="toggle-composer-config" title="Configure task">
          ...
          <div class="input-examples new-task-only">
          ...
          <div class="input-footer">
            ...
            <button id="run-designed-task" data-action="start-designed-task" hidden>Run</button>
          </div>
          <div id="composer-config-panel" class="composer-config-panel" hidden>
            ...
            <div class="composer-config-actions">
              ...
              <button class="secondary-button" data-action="toggle-debug-config">Open full config</button>
            </div>
          </div>
        </div>

        <!-- PIPELINE -->
        <div id="pipeline-card" class="home-main-block"></div>
        ...
        <section id="settings-view" hidden>
          <div class="settings-shell">
            <div class="settings-head">
              <div>
                <div class="settings-eyebrow">Workforce settings</div>
                <h1 id="settings-title">Settings</h1>
                <p id="settings-subtitle">Configure shared runtime capabilities.</p>
              </div>
              <button class="settings-back" data-action="new-task">Back to task</button>
            </div>
            <div id="settings-status" class="settings-status">idle</div>
            <div id="settings-content"></div>
          </div>
        </section>

        <!-- SIMPLE ORG CHART -->
        <section id="simple-org-section" class="simple-only home-main-block">
          <div class="simple-org-header">
            <div class="simple-org-title">
              <h2>Task organization</h2>
              <div class="simple-org-sub" id="simple-org-subtitle">Select a task to inspect its workforce.</div>
            </div>
            <div class="simple-org-actions">
              <button data-action="simple-org-fit">Fit</button>
              <button data-action="simple-org-zoom-out">-</button>
              <button data-action="simple-org-zoom-in">+</button>
            </div>
          </div>
          <div class="simple-org-frame" id="simple-org-frame">
            <div id="simple-org-stage">
              <div id="simple-org-canvas"></div>
            </div>
          </div>
        </section>
            ...
            <div class="task-designer">
              <div class="demo-panel">
                ...
                <div class="demo-actions">
                  ...
                  <button class="secondary-button" id="export-task-trace" data-action="export-task-trace">Export Trace</button>
                  <span class="pill" id="task-trace-export-status">trace idle</span>
                </div>
              </div>
              <div class="muted">Use the composer configuration panel for headcount, budget, and model defaults.</div>
...
function initializeDashboard() {
  ...
  async function deleteTask(taskId) {
    ...
    if (selectedTaskId === taskId) {
      ...
      currentTaskScope = /* @__PURE__ */ new Set();
    }
    await refresh().catch((err) => console.error(err));
  }
  ...
  function renderSettingsView() {
    ...
    if (subtitle) {
      subtitle.textContent = settingsView === "skills" ? "Create centrally managed native Codex and Claude Code skills, then assign them globally or to selected workers." : "Register external MCP servers once, wrap them centrally, and make them available to selected workers.";
    }
  ...
  function renderPipelineCard() {
    ...
    const stepsHtml = PIPELINE_STEPS.map((step, i) => {
      ...
      if (activeIdx > i || finished) {
        lineStyle = "background:#9fd4b7;";
      } else if (isActive) {
        lineStyle = "background:repeating-linear-gradient(90deg,#d9c79c 0 6px,transparent 6px 12px);background-size:14px 2px;animation:wrFlow .7s linear infinite;";
      }
    ...
    const retryHtml = failed ? `<button class="pipe-retry" data-action="design-task-config" style="border:1px solid #ecccc8;background:#fff;color:#b3524b;">Retry</button>` : "";
    host.innerHTML = `
      <div class="pipe-head">
  ...
  function selectedTaskLabel() {
    const task = (lastAllTasks || []).find((item) => item.task_id === selectedTaskId) || (lastTasks || []).find((item) => item.task_id === selectedTaskId);
...
//# sourceURL=/assets/elk.bundled.js`)();
  ...
  function renderSimpleOrgLayout(flattened, layout) {
    ...
    const edgeMarkup = (layout.edges || []).map((edge) => edgePath(edge, layout.positions)).filter(Boolean).map((path) => `<path class="simple-org-edge" d="${esc(path)}"></path>`).join("");
    const cardMarkup = flattened.nodes.map((item) => {
      const pos = layout.positions.get(item.node.id) || { x: 0, y: 0 };
  ...
  function renderHomeHumanReports(reports) {
    ...
    if (!selectedTaskId) {
      host.innerHTML = "";
      const countEl2 = document.getElementById("reports-count");
      if (countEl2) countEl2.textContent = "0";
      const fromEl2 = document.getElementById("reports-from");
      if (fromEl2) fromEl2.textContent = "";
      return;
    }
    const list = reports || [];
  ...
  const DEFAULT_CONFIG = {
    dashboard: { refresh_interval_ms: 5e3, max_visible_agents: 80, collapse_depth: 3, show_idle_activity: true },
  ...
  let collapsedNodes = /* @__PURE__ */ new Set();
  ...
  let currentTaskScope = /* @__PURE__ */ new Set();
  ...
  function sameOutputStream(left, right) {
    return String(left?.agent_id || "") === String(right?.agent_id || "") && String(left?.task_id || "") === String(right?.task_id || "") && String(left?.run_id || "") === String(right?.run_id || "") && String(left?.stream || "output") === String(right?.stream || "output");
  }
  ...
  function startDesignedProgress({ phase, title, detail }) {
    ...
    designedTaskProgressTimer = window.setInterval(renderDesignedProgress, 1e3);
  ...
  function renderDesignedProgress() {
    ...
    if (elapsed) {
      if (designedTaskProgress.startedAt) {
        ...
        elapsed.textContent = `${Math.max(0, Math.round((end - designedTaskProgress.startedAt) / 1e3))}s elapsed`;
      } else {
  ...
  function renderDesignedProgressSteps(activePhase) {
    ...
    return items.map(
      ([phase, label]) => `<span class="progress-step ${phase === activePhase ? "active" : ""}">${esc(label)}</span>`
    ).join("");
  }
  function nextPaint() {
  ...
  function currentCeoAgentId() {
    ...
    const ceo = (agents || []).find((agent) => `${agent.role || ""} ${agent.name || ""}`.toLowerCase().includes("ceo") || `${agent.role || ""}`.toLowerCase().includes("chief executive"));
    return ceo?.id || "ceo";
  }
  ...
  async function startDesignedTask() {
    ...
    if (res.ok && !designedTaskStatus.error) {
      finishDesignedProgress({
        ...
        detail: designedTaskStatus.root_task_id ? `Root task: ${designedTaskStatus.root_task_id}. Watch the org chart, task filter, replay, and live output below.` : "Runtime accepted the task. Watch the org chart, task filter, replay, and live output below."
      });
    } else {
      finishDesignedProgress({
        state: "failed",
  ...
  function renderDraftAgentNode(node) {
    ...
    const childrenMarkup = children.length ? `<ul class="draft-children">${children.map(renderDraftAgentNode).join("")}</ul>` : "";
  ...
  function renderOrgChart() {
    ...
    document.getElementById("org-chart").innerHTML = orgChart.length ? `<ul class="org-tree">${orgChart.map((node) => renderOrgNode(node, 0)).join("")}</ul>` : `<div class="muted">No agents.</div>`;
  }
  ...
  function renderCommunicationPulses() {
    ...
    host.innerHTML = communicationPulses.length ? communicationPulses.map((pulse) => `<div class="communication-pulse ${esc(pulse.kind || "")}">${esc(pulse.text)}</div>`).join("") : `<div class="muted">No recent assignments, discussions, or reports.</div>`;
  }
  ...
  function renderOrgNode(node, depth) {
    ...
    const toggle = hasChildren ? `<button class="tree-toggle" data-action="toggle-node" data-agent-id="${esc(node.id)}" data-depth="${esc(depth)}" title="Toggle reports">${collapsed ? "+" : "-"}</button>` : "";
    const childrenMarkup = hasChildren ? collapsed ? `<ul class="org-children"><li class="org-node"><div class="org-placeholder">${esc(children.length)} direct report(s), ${esc(node.descendant_count || children.length)} total below.</div></li></ul>` : `<ul class="org-children">${children.map((child) => renderOrgNode(child, depth + 1)).join("")}</ul>` : "";
    if (dashboardMode !== "debug") {
      return renderSimpleOrgNode({ node, summary, active, work, toggle, childrenMarkup });
    }
  ...
  function renderHumanReports(reports) {
    ...
    host.innerHTML = items.length ? items.map(renderHumanReportCard).join("") : `<div class="muted">No CEO report to human yet.</div>`;
  }
  function renderHumanReportCard(report) {
    ...
    const nextAction = report.next_action ? `<div class="human-report-next">Next action: ${esc(report.next_action)}</div>` : "";
  ...
  function renderManagerReports(reports) {
    ...
    if (!host) return;
    const items = (reports || []).slice().reverse();
    host.innerHTML = items.length ? items.map(renderManagerReportCard).join("") : `<div class="muted">No internal manager report yet.</div>`;
  }
  function renderManagerReportCard(report) {
    ...
    const workDone = (report.work_done || []).length ? `<div class="manager-report-next">Work done: ${esc((report.work_done || []).join("; "))}</div>` : "";
    const evidence = renderReportEvidence(report.evidence || []);
    const nextAction = report.next_action ? `<div class="manager-report-next">Next action: ${esc(report.next_action)}</div>` : "";
  ...
  function appendAgentEvent(event) {
    ...
    if (event.event_type === "worker_output" || event.event_type === "agent_output") {
      ...
      const status = (event.event_type || "").startsWith("mcp_tool_call_") ? (event.event_type || "").replace("mcp_tool_call_", "") : (event.event_type || "").replace("tool_call_", "");
      activity.tools.push({
        event_id: event.event_id,
        timestamp: event.timestamp,
        event_type: event.event_type,
  ...
  async function refresh() {
    ...
    document.getElementById("updated").textContent = (/* @__PURE__ */ new Date()).toLocaleTimeString();
...
function App() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dashboard-root", dangerouslySetInnerHTML: { __html: dashboardShell } });
}
